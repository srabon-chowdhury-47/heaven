...
export default function AddSale() {
    ...
    try {
      ...
      if (sale.items && sale.items.length > 0) {
        const items = sale.items.map((item) => {
          ...
          // Purchase price now sourced from stock batches (FIFO default), fallback to
          // product's last-known purchase cost if no batch data exists.
  ...
  // --- SEPARATE USEFFECT FOR FETCHING SALE DATA ---
  useEffect(() => {
    if (!isEditing) return;
    ...
    fetchSaleData();
  }, [isEditing, products, customers, stocks]);
  ...
  // Purchase price now comes from Stock Batches (FIFO — the batch that would
  // actually be consumed first on sale), NOT from Product.purchase_cost_bdt.
                      ...
                      <td className="border border-gray-300 px-2 py-1.5 text-center">
                        {batches.length > 0 ? (
                          <select
                            ...
                            className="w-full bg-white border border-gray-300 rounded p-0.5 text-[11px] text-center focus:ring-1 focus:ring-green-500 outline-none font-mono"
                          >
                            {batches.map((b, i) => (
                              <option key={b.id} value={b.id}>
                      ...
                      <td className="border border-gray-300 px-2 py-3 text-center">
                        {item.product && itemBatches.length > 0 ? (
                          <select
                            ...
                            className="w-full bg-white border border-gray-300 rounded p-1 text-xs text-center focus:ring-1 focus:ring-green-500 outline-none font-mono"
                          >
                            {itemBatches.map((b, i) => (
                              <option key={b.id} value={b.id}>
                                ৳{parseFloat(b.mrp).toFixed(2)} ({b.quantity} left{i === 0 ? ", oldest" : ""})
